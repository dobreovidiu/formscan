<?php
	...
	class DocumentUtils
	{
		...
		static public function isHeaderElementText( $text, &$numText, &$numType )
		{
			$text = trim( $text );
			...
			if( $matches && count( $matches ) > 0 && ( count( $matches[0] ) == 2 ) )
			{
				...
				if( !is_bool( $pos ) && ( $pos == 0 ) && ( strlen( $result ) != strlen( $text ) ) )
				{			
					$numText = $result;
					$numType = 1;
					return true;
		...
		// whether header element text
		static public function isHeaderElementText2( $text )
		{
			$text = trim( $text );
